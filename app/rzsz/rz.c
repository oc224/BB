...
char *Copyrrz = "Copyright 2006 Omen Technology Inc All Rights Reserved";
 ...
 */
#ifndef HOWMANY
#define HOWMANY 24	/* for neat debug formatting */
#endif
...
char *argv[];
{
	...
	chkinvok(virgin=argv[0]);
		fprintf(stderr, "%s ready. Type \"%s file ...\" to your modem program\n\r", Progname, Nozmodem?"sb":"sz");
	Tty = 0;  
	npats = 0;
	while (--argc) {
		cp = *++argv;
...
int maxtime;
{
	...
	for (Lastrx=errors=0; errors<RETRYMAX; errors++) {
		vfile("Wcgetsec: firstsec = %d errors = %d", Firstsec, errors);
...
bilge:
			...
			zperr2( "Got bad sector header 0%o ", firstch);
