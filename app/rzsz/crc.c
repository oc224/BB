...
#define _FILE_OFFSET_BITS 64		//	Select default filesystem interface.
/*
...
char *name;
{
	...
	if (ferror(fin)) {
		...
		printf("%08lX %lld ", 0xFFFFFFFF&oldcrc, charcnt);
		if (Block == 128)
			printf("%5lld+%3lld ", charcnt/Block, charcnt%Block);
		if (Block == 1024)
			printf("%5lld+%4lld ", charcnt/Block, charcnt%Block);
		printf(" %s\n", name);
	}
