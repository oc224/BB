...
int amodem_open() {
	...
	if (RS232_OpenComport(amodem_dev_no, amodem_serial_baudrate)) {
...
inline int amodem_puts(const char*msg){
	...
	return RS232_SendBuf(amodem_dev_no,msg,strlen(msg));
}
...
int amodem_gets(char* buf,int size){
	...
	n=RS232_PollComport(amodem_dev_no,dump,BUFSIZE);
	if (n<1){
		return FAIL;
	}
...
int amodem_upload_file(const char *fname){
	...
	amodem_puts(buf);
	amodem_wait_info(NULL,SERIAL_TIMEOUT,NULL,BUFSIZE);
	...
	// wait for the copy
	if (amodem_wait_ack("ok",60000)==FAIL){//TODO estimate time, proper value
